  ...
  return (
    <section className="max-w-5xl p-6 mx-auto">
      <div className="bg-white border p-2 min-h-[400px]">
        ...
        {newArticle.content.map((contentItem, index) => (
          <div key={index} className="p-4 mb-2">
            ...
            {contentItem.type === "main" && (
              ...
              >
                Tilføj "main" paragraph + headline
              </button>
            )}
          </div>
        ))}
        <button
          type="button"
          onClick={() => addContentItem("new")}
          className="px-3 py-2 mb-2 border"
        >
          Add Content Item
        </button>
        ...
        <label className="flex items-center gap-2 mt-4">
          <input
            type="checkbox"
            checked={newArticle.isLandingpage}
      ...
      {articles.map((article, index) => (
        <div key={article._id} className="p-2 mb-20 bg-white border">
          ...
          <input
            type="text"
            value={article.title}
            onChange={(e) => handleChange(article._id!, "title", e.target.value)}
            className="w-full px-3 py-2 mb-2 border"
            placeholder="Title"
          />
          ...
          <input
            ...
            onChange={(e) => handleChange(article._id!, "author", e.target.value)}
            className="w-full px-3 py-2 mb-2 border"
            placeholder="Author"
          />
          ...
          {article.content.map((contentItem, index) => (
            <div key={index} className="p-4 mb-2 border rounded-md">
              ...
              {contentItem.type === "main" && (
                <button
                  type="button"
                  onClick={() => addContentBodyItem(article, index)}
                  className="px-2 py-1 mt-1 border"
                >
                  Add Nested Content
                </button>
          ...
          <label className="flex items-center gap-2 mb-2">
            <input
              type="checkbox"
              checked={article.isLandingpage}
              onChange={() => toggleLandingPage(article)}
            />
            Landing Page
          </label>
